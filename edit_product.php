      ...
      <div class="flex items-center gap-4">
        <div class="flex items-center gap-3 cursor-pointer hover:opacity-80 transition-opacity">
          ...
          <div class="relative">
            ...
            <span class="absolute bottom-0 right-0 size-3.5 bg-success border-2 border-white rounded-full"></span>
          </div>
        </div>
      </div>
    </header>

    <form id="productForm" action="update_product.php" method="POST" enctype="multipart/form-data" class="flex-1 overflow-y-auto p-4 md:p-8">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <div class="flex items-center gap-3 w-full sm:w-auto">
          <button type="button" onclick="window.history.back()" class="flex-1 sm:flex-none px-6 py-2.5 rounded-xl font-semibold text-secondary bg-white border border-border hover:bg-muted transition-colors text-center cursor-pointer">
            Cancel
          </button>
          <button type="button" onclick="saveProduct()" class="flex-1 sm:flex-none px-6 py-2.5 rounded-xl font-semibold text-white bg-primary hover:bg-primary-hover shadow-lg shadow-primary/20 transition-all text-center flex items-center justify-center gap-2 cursor-pointer">
        ...
        <div class="lg:col-span-2 flex flex-col gap-6 md:gap-8">
          ...
          <div class="bg-white rounded-2xl border border-border p-6 shadow-sm">
            ...
            <div class="space-y-5">
              <div>
                ...
                <input type="text" id="inputProductName" name="product_name" value="<?= htmlspecialchars($product['name']) ?>" class="w-full bg-muted border border-border rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all text-foreground" placeholder="Enter product name">
              </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  <input type="number" id="inputBasePrice" name="price" value="<?= $product['price'] ?>" step="0.01" class="w-full bg-muted border border-border rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all text-foreground">
                </div>
              </div>

              <div>
                <label class="block text-sm font-medium text-foreground mb-2">Description</label>
                <textarea id="inputDescription" name="description" rows="4" class="w-full bg-muted border border-border rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all text-foreground resize-none" placeholder="Describe your product..."><?= htmlspecialchars($product['description']) ?></textarea>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-2xl border border-border p-6 shadow-sm">
            <div class="flex items-center justify-between mb-6">
              ...
              <button type="button" onclick="addSpecRow()" class="text-sm font-semibold text-primary hover:text-primary-hover flex items-center gap-1 bg-primary/10 px-3 py-1.5 rounded-lg transition-colors cursor-pointer">
              ...
              <?php foreach ($specs as $row): ?>
                <div class="dynamic-row flex flex-col sm:flex-row items-start sm:items-center gap-3 bg-muted/50 p-3 sm:p-0 sm:bg-transparent rounded-xl border border-border sm:border-none">
                  <div class="w-full sm:flex-1">
                    ...
                    <input type="text" name="spec_names[]" value="<?= htmlspecialchars($row['spec_name']) ?>" placeholder="e.g. Material" class="w-full bg-white sm:bg-muted border border-border rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all text-sm">
                  </div>
                  <div class="w-full sm:flex-1">
                    <label class="sm:hidden text-xs font-medium text-secondary mb-1 block">Value</label>
                    <input type="text" name="spec_values[]" value="<?= htmlspecialchars($row['spec_value']) ?>" placeholder="e.g. Cotton" class="w-full bg-white sm:bg-muted border border-border rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all text-sm">
                  </div>
                  <button type="button" onclick="removeRow(this)" class="w-full sm:w-10 h-10 flex items-center justify-center rounded-xl text-error hover:bg-error/10 transition-colors shrink-0 border border-error/20 sm:border-none mt-2 sm:mt-0 cursor-pointer" title="Remove">
                  ...
                  <div class="flex gap-4 sm:gap-7 pr-4">
                    <div class="shrink-0">
                      <label class="file-upload-wrapper relative w-20 h-20 border-2 border-dashed border-border rounded-xl flex flex-col items-center justify-center bg-white hover:bg-muted transition-colors overflow-hidden group/img cursor-pointer">
                        <input type="file" name="variant_images[<?= $index ?>]" accept="image/*" onchange="previewImage(this)">
                        
                        <input type="hidden" name="old_variant_images[<?= $index ?>]" value="<?= $row['product_image'] ?>">
                        
                        <img src="<?= !empty($row['product_image']) ? 'storage/image/' . $row['product_image'] : 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=200&h=200&fit=crop'; ?>" class="absolute inset-0 w-full h-full object-cover z-10" alt="Variant image">
                    ...
                    <div class="flex-1 flex flex-col gap-3 justify-center">
                      <div>
                        <input type="text" name="variant_colors[<?= $index ?>]" value="<?= htmlspecialchars($row['color_name']) ?>" placeholder="Color Name (e.g. Red)" class="w-full bg-white border border-border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all text-sm font-medium">
                      </div>
                      <div class="flex items-center gap-2">
                        <span class="text-xs text-secondary font-medium w-12">Stock:</span>
                        <input type="number" name="variant_stocks[<?= $index ?>]" value="<?= $row['color_stok'] ?>" placeholder="0" class="w-24 bg-white border border-border rounded-lg px-3 py-1.5 outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all text-sm">
                      </div>
                    </div>
                  </div>
                </div>
...
<script>
  ...
  function addSpecRow() {
    ...
    row.innerHTML = `
      <div class="w-full sm:flex-1">
        <label class="sm:hidden text-xs font-medium text-secondary mb-1 block">Name</label>
        <input type="text" name="spec_names[]" placeholder="e.g. Weight" class="w-full bg-white sm:bg-muted border border-border rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all text-sm">
      </div>
      <div class="w-full sm:flex-1">
        <label class="sm:hidden text-xs font-medium text-secondary mb-1 block">Value</label>
        <input type="text" name="spec_values[]" placeholder="e.g. 250g" class="w-full bg-white sm:bg-muted border border-border rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all text-sm">
      </div>
      <button type="button" onclick="removeRow(this)" class="w-full sm:w-10 h-10 flex items-center justify-center rounded-xl text-error hover:bg-error/10 transition-colors shrink-0 border border-error/20 sm:border-none mt-2 sm:mt-0 cursor-pointer" title="Remove">
        <i data-lucide="trash-2" class="size-4 sm:size-5"></i>
        <span class="sm:hidden ml-2 text-sm font-medium">Remove Spec</span>
    ...
    lucide.createIcons();
    requestAnimationFrame(() => { row.classList.remove('opacity-0', 'translate-y-2'); });
  }
  ...
  function addVariantRow() {
    ...
    row.innerHTML = `
      <button type="button" onclick="removeRow(this)" class="absolute top-2 right-2 size-8 flex items-center justify-center rounded-lg text-secondary hover:text-error hover:bg-error/10 transition-colors cursor-pointer" title="Remove Variant">
        <i data-lucide="x" class="size-4"></i>
      </button>
      <div class="flex gap-4">
        <div class="shrink-0">
          <label class="file-upload-wrapper relative w-20 h-20 border-2 border-dashed border-border rounded-xl flex flex-col items-center justify-center bg-white hover:bg-muted transition-colors overflow-hidden group/img cursor-pointer">
            <input type="file" name="variant_images[\${index}]" accept="image/*" onchange="previewImage(this)">
            <input type="hidden" name="old_variant_images[\${index}]" value="">
        ...
        <div class="flex-1 flex flex-col gap-3 justify-center">
          <div>
            <input type="text" name="variant_colors[\${index}]" placeholder="Color Name (e.g. Blue)" class="w-full bg-white border border-border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all text-sm font-medium">
          </div>
          <div class="flex items-center gap-2">
            <span class="text-xs text-secondary font-medium w-12">Stock:</span>
            <input type="number" name="variant_stocks[\${index}]" placeholder="0" class="w-24 bg-white border border-border rounded-lg px-3 py-1.5 outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all text-sm">
          </div>
        </div>
      </div>
    `;
  ...
  function showToast(msg, type='success') {
    ...
    t.innerHTML = `<i data-lucide="${icon}" class="size-5 shrink-0"></i><span class="flex-1">${msg}</span>`;
